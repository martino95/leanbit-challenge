...
class Manager():
  ...
  def add_projects(self):
    if self.data.get('projects') == None:
      raise ValueError('Data file does not contain projects. Expected a periods list identified by the key "projects"')
  ...
  def add_country_holidays(self, country):
    sy = min([p['since'].year for p in self.periods]+[p['since'].year for p in self.projects])
    ey = max([p['until'].year for p in self.periods]+[p['until'].year for p in self.projects])
  ...
  def get_feasibility_report(self,outf):
    ...
    for project in self.projects:
      ...
      for devel in self.developers:
        devtotal, devwork, devwend, devholi = check_developer_availability(devel, project)
        available_workdays += devwork
      feasibility = available_workdays >= project['effort_days']
      total, work, wend, holi = check_dates(project['since'], project['until'], self.holidays)
      report_list.append(NoIndent(OrderedDict([('project_id', project['id']), 
  			('total_days', total), 
  			('workdays', work), 
  			('weekend_days', wend), 
  			('holidays', holi),
